...
export function ResetPasswordProviders({ name, providers }: { name: string; providers: string[] }) {
  return (
    <EmailLayout>
      <p>Hello {name},</p>
      <p>
        We have received a requested to change your password. Our records show that you registered
        for the UCI Machine Learning Repository using the following providers:
      </p>
      <ul>{providers && providers.map((provider) => <li key={name}>{provider}</li>)}</ul>
      <p>
        To access you account, try{" "}
        <a href={path.join(env.ORIGIN, SIGN_IN_ROUTE)} className="text-blue-500 hover:underline">
          logging in
        </a>{" "}
